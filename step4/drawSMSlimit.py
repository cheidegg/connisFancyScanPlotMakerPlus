## somebody else's script producing all histograms, interpolation, exclusion limits etc.
## command: python drawSMSlimit.py <textFleWithLimits> <rootFileWithXS>
#!/usr/bin/env python
from math import *
import os, commands
...
if len(argv)<3:
    print "Usage: "+argv[0]+" fileWithLimits.txt xsfile.root"
    exit(1)

INPUT = argv[1]
xsfile = argv[2]

models   = ["T1bbbb", "T1tttt","T1qqqq","T2qq","T2bb","T2tt", "TChiNeu","TNeuNeu"]
model = "mymodel"
for m in models:
    if m in INPUT:
        model = m
...
mass1 = "mChi" if "TChiNeu" in model else "mGlu" if "T1" in model else "mSq" if model=="T2qq" else "mSb" if model=="T2bb" else "mSt" if model=="T2tt" else "m1"
mass2 = "mLSP"
